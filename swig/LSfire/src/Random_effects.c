...
//                 #include "lsm_file.h"
// which, in turn, #include "TurbFireSpot_config.h"
...
int init_grid()
{    
	TurbFireSpot_REAL x_lo[2] ;
	TurbFireSpot_REAL x_hi[2] ;
    ...
    gridTFS = createGridSetDx(2, dx, x_lo, x_hi, 1) ;
    
    
    if (Nx != gridTFS->grid_dims[0])
    {
		printf("*** WARNING: Nx ricalcolato [%d --> %d] ***\n", Nx, gridTFS->grid_dims[0]) ;
		Nx = gridTFS->grid_dims[0] ;
	}
	
    if (Ny != gridTFS->grid_dims[1]) 
    {
		printf("*** WARNING: Ny ricalcolato [%d --> %d] ***\n", Ny, gridTFS->grid_dims[1]) ;
		Ny = gridTFS->grid_dims[1] ;
	}
	
	if (x_1 != gridTFS->x_hi[0])
    {
		printf("*** WARNING: x_1 ricalcolato [%f --> %f] ***\n", x_1, gridTFS->x_hi[0]) ;
		x_1 = gridTFS->x_hi[0] ;
	}
	
	if (y_1 != gridTFS->x_hi[1])
    {
    ...
    Nx_g = gridTFS->grid_dims_ghostbox[0] ;
    Ny_g = gridTFS->grid_dims_ghostbox[1] ;
    N_g  = gridTFS->num_gridpts ;
    
    x_0_g = gridTFS->x_lo_ghostbox[0] ; x_1_g = gridTFS->x_hi_ghostbox[0] ;
    y_0_g = gridTFS->x_lo_ghostbox[1] ; y_1_g = gridTFS->x_hi_ghostbox[1] ;
    
    // gridTFS->grid_dims[0]            Nx
    // gridTFS->grid_dims[1]            Ny
    // gridTFS->grid_dims_ghostbox[0]   Nx_g
    // gridTFS->grid_dims_ghostbox[1]   Ny_g
    // gridTFS->num_gridpts             Nx_g*Ny_g
    
    // indixes of the extended domain (including ghostboxes)
    i0_g = gridTFS->ilo_gb ; i1_g = gridTFS->ihi_gb ;
    j0_g = gridTFS->jlo_gb ; j1_g = gridTFS->jhi_gb ; 
	
	// indexes of the domain for the calculation of derivatives (fillboxes)
	i0_f = gridTFS->ilo_fb ; i1_f = gridTFS->ihi_fb ; 
	j0_f = gridTFS->jlo_fb ; j1_f = gridTFS->jhi_fb ; 
...
int init_tfs()
{    
    dataTFS = allocateTFSDataArrays() ;
    
    allocateMemoryForTFSDataArrays(dataTFS, gridTFS) ;
        
    if (flag_init_circle)
        createCircle(dataTFS->phi, init_circle_xc, init_circle_yc, 
			init_circle_r, init_circle_inside, gridTFS) ;
            
	if (flag_init_rectangle)
		createRectangle(dataTFS->phi, init_rectangle_x0, init_rectangle_y0,
			init_rectangle_Lx, init_rectangle_Ly, 
            init_rectangle_inside, gridTFS) ;
    
    for (int idx = 0; idx < N_g; idx++)
    {
		//if (dataTFS->phi[idx] < 0.) printf("%f, ",dataTFS->phi[idx]);
		//if (dataTFS->phi[idx] > 0.5) printf("%f, ",dataTFS->phi[idx]);
		
		if (dataTFS->phi[idx] < 0.) {
			dataTFS->phi[idx] = 0. ;
        }
		else {
			dataTFS->phi[idx] = 1.;
		...
		//dataTFS->phi[n] = 1.0 - dataTFS->phi[n] ;
	}
	    
    	nx     = (TurbFireSpot_REAL*) calloc(N_g, sizeof(TurbFireSpot_REAL)) ;
	ny     = (TurbFireSpot_REAL*) calloc(N_g, sizeof(TurbFireSpot_REAL)) ;
	phieff = (TurbFireSpot_REAL*) calloc(N_g, sizeof(TurbFireSpot_REAL)) ;
	phi_ff = (TurbFireSpot_REAL*) calloc(N_g, sizeof(TurbFireSpot_REAL)) ;
	psi    = (TurbFireSpot_REAL*) calloc(N_g, sizeof(TurbFireSpot_REAL)) ;
	//psi    = (double*) calloc(N_g, sizeof(double)) ;


	if (flag_save_tburn_phi)
		tburn_phi    = (TurbFireSpot_REAL*) calloc(N_g, sizeof(TurbFireSpot_REAL)) ;
	if (flag_save_tburn_phieff)
		tburn_phieff = (TurbFireSpot_REAL*) calloc(N_g, sizeof(TurbFireSpot_REAL)) ;
	if (flag_save_tburn_psi)
		tburn_psi    = (TurbFireSpot_REAL*) calloc(N_g, sizeof(TurbFireSpot_REAL)) ;
	
	
	for (int idx = 0; idx < N_g; idx++)
	{
		phieff[idx] = dataTFS->phi[idx] ;
		phi_ff[idx] = 0.0;
		psi[idx]    = 0.0 ;
	...
	if (flag_kernel_Gauss_num)
	{
		kernel     = (TurbFireSpot_REAL*) calloc(Nr, sizeof(TurbFireSpot_REAL)) ;
		kernel_old = (TurbFireSpot_REAL*) calloc(Nr, sizeof(TurbFireSpot_REAL)) ;
			
		r  = (TurbFireSpot_REAL*) calloc(Nr, sizeof(TurbFireSpot_REAL)) ; 
		ri = (TurbFireSpot_REAL*) calloc(Nr-1, sizeof(TurbFireSpot_REAL)) ;
		Dr = (TurbFireSpot_REAL*) calloc(Nr-1, sizeof(TurbFireSpot_REAL)) ;
		DR = (TurbFireSpot_REAL*) calloc(Nr, sizeof(TurbFireSpot_REAL)) ;
		ka = (TurbFireSpot_REAL*) calloc(Nr, sizeof(TurbFireSpot_REAL)) ;
		kb = (TurbFireSpot_REAL*) calloc(Nr-1, sizeof(TurbFireSpot_REAL)) ;
		kc = (TurbFireSpot_REAL*) calloc(Nr-1, sizeof(TurbFireSpot_REAL)) ;
		th_m = (TurbFireSpot_REAL*) calloc(Nr, sizeof(TurbFireSpot_REAL)) ;
		th_y = (TurbFireSpot_REAL*) calloc(Nr, sizeof(TurbFireSpot_REAL)) ; 
		th_ka = (TurbFireSpot_REAL*) calloc(Nr, sizeof(TurbFireSpot_REAL)) ;
		th_kb = (TurbFireSpot_REAL*) calloc(Nr, sizeof(TurbFireSpot_REAL)) ; 
		th_kc = (TurbFireSpot_REAL*) calloc(Nr, sizeof(TurbFireSpot_REAL)) ; 
	}
	else
	{
		flag_save_kernel = 0 ;
...
int init_fire() {
	...
	ignMap   = (TurbFireSpot_REAL*) calloc(N_map, sizeof(TurbFireSpot_REAL)) ;
	//flameMap = (TurbFireSpot_REAL*) calloc(N_map, sizeof(TurbFireSpot_REAL)) ;
	slpMap   = (TurbFireSpot_REAL*) calloc(N_map, sizeof(TurbFireSpot_REAL)) ;
	aspMap   = (TurbFireSpot_REAL*) calloc(N_map, sizeof(TurbFireSpot_REAL)) ;
	wspdMap  = (TurbFireSpot_REAL*) calloc(N_map, sizeof(TurbFireSpot_REAL)) ;
	wdirMap  = (TurbFireSpot_REAL*) calloc(N_map, sizeof(TurbFireSpot_REAL)) ;
	m1Map    = (TurbFireSpot_REAL*) calloc(N_map, sizeof(TurbFireSpot_REAL)) ;
	m10Map   = (TurbFireSpot_REAL*) calloc(N_map, sizeof(TurbFireSpot_REAL)) ;
	m100Map  = (TurbFireSpot_REAL*) calloc(N_map, sizeof(TurbFireSpot_REAL)) ;
	mherbMap = (TurbFireSpot_REAL*) calloc(N_map, sizeof(TurbFireSpot_REAL)) ;
	mwoodMap = (TurbFireSpot_REAL*) calloc(N_map, sizeof(TurbFireSpot_REAL)) ;
	
	ros = (TurbFireSpot_REAL*) calloc(N_g, sizeof(TurbFireSpot_REAL) ) ;
...
int get_RoS(TurbFireSpot_REAL t)
{
    ros_max = 0.0 ;
 ...
 **********************************************************************/
int lsm2_RK2(TurbFireSpot_REAL dt, TurbFireSpot_REAL t) 
{
	//int iflagtest = 0 ;		
	...
	{			
        ...
        LSM2_LSM2D_ZERO_OUT_LEVEL_SET_EQN_RHS(
            dataTFS->lse_rhs, &i0_g, &i1_g, &j0_g, &j1_g, &Nthreads) ;

		// calculate the upwind approximation of Grad(phi)
        
		if (istage==1)
			LSM2_LSM2D_HJ_ENO1(
                dataTFS->phi_x_plus, dataTFS->phi_y_plus, &i0_g, 
				&i1_g, &j0_g, &j1_g, dataTFS->phi_x_minus, 
				dataTFS->phi_y_minus, &i0_g, &i1_g, &j0_g, &j1_g,
				dataTFS->phi, &i0_g, &i1_g, &j0_g, &j1_g, dataTFS->D1, 
			...
			LSM2_LSM2D_HJ_ENO1(
				...
				&i1_g, &j0_g, &j1_g, dataTFS->phi_x_minus, 
				dataTFS->phi_y_minus, &i0_g, &i1_g, &j0_g, &j1_g,
				dataTFS->phi_stage1,	&i0_g, &i1_g, &j0_g, &j1_g, 
				dataTFS->D1,	&i0_g, &i1_g, &j0_g, &j1_g, &i0_f, &i1_f,
				&j0_f, &j1_f, &dx, &dy, &Nthreads) ;
			
		LSM2_LSM2D_AVERAGE_GRAD_PHI(
            dataTFS->phi_x, dataTFS->phi_y, &i0_g,
			&i1_g, &j0_g, &j1_g, dataTFS->phi_x_plus, dataTFS->phi_y_plus,
			&i0_g, &i1_g, &j0_g, &j1_g, dataTFS->phi_x_minus, 
			dataTFS->phi_y_minus, &i0_g, &i1_g, &j0_g, &j1_g, &i0_f, 
			&i1_f, &j0_f, &j1_f, &Nthreads) ;
		
		// compute the normal to the surface phi=0
		LSM2_LSM2D_COMPUTE_UNIT_NORMAL(
            nx, ny, &i0_g, &i1_g, &j0_g, &j1_g,
			dataTFS->phi_x, dataTFS->phi_y, &i0_g, &i1_g, &j0_g, &j1_g,
		...
		//signedLinearExtrapolationBC(ny, gridTFS, 9) ;
		//signedLinearExtrapolationBC(ny, gridTFS, 9) ;
		...
		LSM2_LSM2D_ADD_NORMAL_VEL_TERM_TO_LSE_RHS(
            dataTFS->lse_rhs, &i0_g, 
			&i1_g, &j0_g, &j1_g, dataTFS->phi_x_plus, dataTFS->phi_y_plus,
			&i0_g, &i1_g, &j0_g, &j1_g, dataTFS->phi_x_minus, 
			dataTFS->phi_y_minus, &i0_g, &i1_g, &j0_g, &j1_g, ros, &i0_g,
			&i1_g, &j0_g, &j1_g, &i0_f, &i1_f, &j0_f, &j1_f, &Nthreads) ;
		
		//~ for (int idx = 0; idx < N_g; idx++)
			//~ dataTFS->lse_rhs[idx] += 0.0 * (dataTFS->phi_x_plus[idx] - 
				//~ dataTFS->phi_x_minus[idx] + dataTFS->phi_y_plus[idx] - 
				//~ dataTFS->phi_y_minus[idx]) ;                               // why?
		        
		// advance phi and fill boundary cells
		if (istage==1)  
		{
			LSM2_LSM2D_TVD_RK2_STAGE1(dataTFS->phi_stage1, &i0_g, &i1_g,
				&j0_g, &j1_g, dataTFS->phi, &i0_g, &i1_g, &j0_g, &j1_g,
				dataTFS->lse_rhs, &i0_g, &i1_g, &j0_g, &j1_g, &i0_f, 
				&i1_f, &j0_f, &j1_f, &dt, &Nthreads) ;
						
			LSM2_signedLinearExtrapolationBC(
                dataTFS->phi_stage1, 
                &i0_g, &i1_g, &j0_g, &j1_g, &i0_f, &i1_f, &j0_f, &j1_f) ;
			//signedLinearExtrapolationBC(dataTFS->phi_stage1, gridTFS, 9) ;
		}
		else 
		{
			LSM2_LSM2D_TVD_RK2_STAGE2(
                dataTFS->phi_next, &i0_g, &i1_g, &j0_g, 
				&j1_g, dataTFS->phi_stage1, &i0_g, &i1_g, &j0_g, &j1_g,
				dataTFS->phi, &i0_g, &i1_g, &j0_g, &j1_g, 
				dataTFS->lse_rhs, &i0_g, &i1_g, &j0_g, &j1_g, &i0_f, 
				&i1_f, &j0_f, &j1_f, &dt, &Nthreads) ;
			...
			LSM2_signedLinearExtrapolationBC(
                dataTFS->phi_next, 
                &i0_g, &i1_g, &j0_g, &j1_g, &i0_f, &i1_f, &j0_f, &j1_f) ;
			//signedLinearExtrapolationBC(dataTFS->phi_next, gridTFS, 9) ;
		}       
 ...
 int update_phi()
{
	...
	//#pragma omp parallel
    for (int idx = 0; idx < N_g; idx++) 
		dataTFS->phi[idx] = dataTFS->phi_next[idx] ;
...
//					phieff[idxa] += dataTFS->phi[idxb] * e ;	
...
//    #pragma omp parallel for 
    for (int ia = i0_g; ia < i1_g + ic; ia++)
    {
        ...
        {
            int idxa = ia + ja * Nx_g ;
        //if (t > 16.1 && t < 16.5 && dataTFS->phi[idxa] < 0.99 && dataTFS->phi[idxa] > 0 )
        //   { printf("%d %d %f \n",ia,ja,dataTFS->phi[idxa]) ;
        // }
            double xa = x_0_g + ia * dx + d_center_x ;
            double ya = y_0_g + ja * dy + d_center_y ;
            ...
            double normalizzatore = 0.0 ;
                        ...
                        for (int jb = j0b; jb < j1b + ic; jb++)
                        {
                                for (int ib = i0b; ib < i1b + ic; ib++)
                                {
                                        ...
                                        //if (dataTFS->phi[idxb] > PHI_THRESHOLD) continue ;
            ...
            //phieff[idxa] *= dx * dy ;
            //printf("%d %d %f %f %f \n",ia,ja,phieff[idxa],normalizzatore,dataTFS->phi[idxa]);      

            phieff[idxa] /= normalizzatore  ;
            if(ya*FT2M < 2505.0 && ya*FT2M  > 2495.0)fprintf(fphi,"%f %f %f %f %f\n",xa*FT2M,ya*FT2M,phi_ff[idxa],phieff[idxa],dataTFS->phi[idxa]);
            //printf("\n %f",phieff[idxa]);
    }
    }
	fclose(fphi);
 ...
 **********************************************************************/
int get_psi(TurbFireSpot_REAL dt, TurbFireSpot_REAL t)
...
{	
	...
	{
		for (int idx = 0; idx < N_g; idx++)
			if ( dataTFS->phi[idx] >= PHI_THRESHOLD ) 
...
unsigned int check_fire_border()
{
	...
	for (int i = i0_i; i < i1_i + ic ; i++)
	{
		...
		//if (flag_backward_compatibility)
		//{
		//	if (dataTFS->phi[i+j0_f*Nx_g] < PHI_THRESHOLD) flag_fire_at_border = 1 ;
		//	if (dataTFS->phi[i+j1_f*Nx_g] < PHI_THRESHOLD) flag_fire_at_border = 1 ;
		//}
		//else
		//{
	...
	for (int j = j0_i; j < j1_i + ic ; j++)
	{
		...
		//if (flag_backward_compatibility)
		//{
		//	if (dataTFS->phi[i0_f+j*Nx_g] < PHI_THRESHOLD) flag_fire_at_border = 1 ;
		//	if (dataTFS->phi[i1_f+j*Nx_g] < PHI_THRESHOLD) flag_fire_at_border = 1 ;
		//}
		//else
		//{
...
int save_array_k_to_file(char* filename, int Niter)   
{
    ...
    {			
		...
		for (int i = 0; i < Nr; i++) 
			fwrite(r+i, sizeof(TurbFireSpot_REAL), 1, fp) ;
		
		for (int i = 0; i < Nr; i++) 
			fwrite(kernel+i, sizeof(TurbFireSpot_REAL), 1, fp) ;
 ...
 **********************************************************************/
int save_array2d_to_file(char* filename, TurbFireSpot_REAL* data, int Niter, char type)   
{
	char filename_full[MAX_CHAR_NFILE] ;
	FILE *fp ;
    int flag_transpose ;
		...
		for (int j = j0_save; j <= j1_save; j++) 
		{
			for (int i = i0_save; i <= i1_save; i++) 
			{
				...
				if (type=='d')
					...
					fwrite(data+idx, sizeof(TurbFireSpot_REAL), 1, fp) ;
			}
		} 
 ...
 * save data                                                          *
 **********************************************************************/
int save_dataiter_to_file(int Niter, TurbFireSpot_REAL t)
{
	char filename_full[MAX_CHAR_NFILE] ;
	...
	if (flag_saveformat_binary)
	{
		...
		fwrite(&t, sizeof(TurbFireSpot_REAL), 1, fp ) ;
		fclose(fp) ;
	}
		
	if (flag_save_phi) 
		save_array2d_to_file(filename_phi, dataTFS->phi, Niter, 'f') ;
 ...
 **********************************************************************/
int save_datapost_to_file(int Niter, TurbFireSpot_REAL t)   
{
	FILE *fp ;
	char filename_full[MAX_CHAR_NFILE] ;
...
int main(int argc, char **argv)
{
    ...
    clock_t tic, toc ;    
    time_t  wtic, wtoc ;
    TurbFireSpot_REAL t, dt, dt_cfl, t_old ;
    //int c ;
    FILE *fp ;
	...
	if (flag_save_fuelMap) 
	{
		...
		//save_array2d_to_file(filename_fuelMap, (TurbFireSpot_REAL*)fuelMap, 0, 'd') ;
