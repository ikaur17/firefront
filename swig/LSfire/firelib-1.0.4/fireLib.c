    ...
    size_t         model;       /* fuel model id number         [0-maxModels] */
{
    ...
    if ( ! Fire_FuelModelExists(catalog,model) )
    {
        sprintf(FuelCat_Error(catalog),
            "Fire_FuelCombustion(): fuel model %zu doesn't exist in fuel catalog \"%s\".",    /* %d -> %zu (by Undy) */
    ...
    double  moisture[FIRE_MCLASSES]; /* array of fuel moistures   (fractions) */
{
    ...
    {
        sprintf(FuelCat_Error(catalog),
            "Fire_SpreadNoWindNoSlope(): fuel model %zu doesn't exist in fuel catalog \"%s\".",  /* %d -> %zu (by Undy) */
            model, FuelCat_Name(catalog));
        return (FuelCat_Status(catalog) = FIRE_STATUS_ERROR);
    }
    ...
    Fuel_SpreadMax(catalog,model) = Fuel_Spread0(catalog,model);
    Fuel_SpreadAny(catalog,model) = Fuel_Spread0(catalog,model);
    Fuel_BackingRate(catalog,model) = Fuel_Spread0(catalog,model);
 ...
 *          Fuel_BackingRate(catalog, model)  = backing;
 *
...
int
Fire_SpreadWindSlopeMax ( catalog, model, windFpm, windDeg, slope, aspect )
    FuelCatalogPtr catalog;     /* FuelCatalogData instance pointer           */
    size_t  model;              /* fuel model number            [0-maxModels] */
    ...
    double  aspect;             /* aspect (downslope)   azimuth (compass deg) */
{
    ...
    double x, y, al, a;
    double maxWind, effectiveWind, lwRatio, eccentricity, backing;
    ...
    {
        sprintf(FuelCat_Error(catalog),
            "Fire_SpreadMax(): fuel model %zu doesn't exist in fuel catalog \"%s\".",   /* %d -> %zu (by Undy) */
            model, FuelCat_Name(catalog));
        return (FuelCat_Status(catalog) = FIRE_STATUS_ERROR);
    }
    ...
    if ( effectiveWind > Smidgen )
    {
        ...
        if ( lwRatio > 1.00001 )                        /* BUG FIX */
            eccentricity = sqrt(lwRatio * lwRatio - 1.0) / lwRatio;
    }

    /* Backing spread rate. */
    backing = spreadMax * (1.-eccentricity) / (1.+eccentricity);
...
int
Fire_SpreadAtAzimuth ( catalog, model, azimuth, which )
    FuelCatalogPtr catalog;     /* FuelCatalogData instance pointer           */
    size_t  model;              /* fuel model number            [0-maxModels] */
    ...
    size_t  which;      /* FIRE_NONE | FIRE_BYRAMS | FIRE_FLAME | FIRE_SCORCH */
{
    ...
    {
        sprintf(FuelCat_Error(catalog),
            "Fire_SpreadAtAzimuth(): fuel model %zu doesn't exist in fuel catalog \"%s\".",   /* %d -> %zu (by Undy) */
            model, FuelCat_Name(catalog));
        return (FuelCat_Status(catalog) = FIRE_STATUS_ERROR);
    }
    ...
    else
    {
        ...
        denom = 1. - Fuel_Eccentricity(catalog,model) * cos(radians); /* BUG FIX */
        if ( denom > Smidgen )
        {
            Fuel_SpreadAny(catalog,model)
                = Fuel_SpreadMax(catalog,model)
                * (1. - Fuel_Eccentricity(catalog,model)) / denom;
        }
        else
        {
            Fuel_SpreadAny(catalog,model) = Fuel_Spread0(catalog,model); /* BUG FIX */
        }
    }
    Fuel_AzimuthAny(catalog,model) = azimuth;
...
int
Fire_FlameScorch ( catalog, model, which )
    FuelCatalogPtr catalog;     /* FuelCatalogData instance pointer           */
    size_t  model;              /* fuel model number            [0-maxModels] */
    size_t  which;      /* FIRE_NONE | FIRE_BYRAMS | FIRE_FLAME | FIRE_SCORCH */
{
    ...
    {
        sprintf(FuelCat_Error(catalog),
            "Fire_FlameScorch(): fuel model %zu doesn't exist in fuel catalog \"%s\".",    /* %d -> %zu (by Undy) */
            model, FuelCat_Name(catalog));
        return (FuelCat_Status(catalog) = FIRE_STATUS_ERROR);
    }
    ...
    double  flameStep;          /* flame length step value per class          */
{
    ...
    {
        sprintf(FuelCat_Error(catalog),
            "Fire_FlameLengthTable(): unable to allocate flame length table with %zu classes of %f feet.",   /* %d -> %zu (by Undy) */
    ...
    size_t maxModels;           /* maximum modelId allowed in this catalog */
{
    ...
    {
        fprintf(stderr,
            "Fire_FuelCatalogCreate(): unable to allocate fuel catalog \"%s\" with %zu fuel models.\n",    /* %d -> %zu (by Undy) */
            name, maxModels);
...
int
Fire_FuelModelCreate (catalog, model, name, desc, depth, mext, adjust, maxParticles)
    ...
    size_t  maxParticles;       /* maximum number of fuel model particles     */
{
    ...
    {
        sprintf(FuelCat_Error(catalog),
            "Fire_FuelModelCreate(): fuel model \"%s\" number %zu exceeds fuel catalog \"%s\" range [0..%zu].",   /* %d -> %zu twice (by Undy) */
    ...
    {
        sprintf(FuelCat_Error(catalog),
            "Fire_FuelModelCreate(): fuel model \"%s\" number %zu depth %5.4f is too small.",  /* %d -> %zu (by Undy) */
            name, model, depth);
        return (FuelCat_Status(catalog) = FIRE_STATUS_ERROR);
    }
    ...
    {
        sprintf(FuelCat_Error(catalog),
            "Fire_FuelModelCreate(): fuel model \"%s\" number %zu extinction moisture %5.4f is too small.",   /* %d -> %zu (by Undy) */
            name, model, mext);
        return (FuelCat_Status(catalog) = FIRE_STATUS_ERROR);
    }
                ...
                (PartPtr *) calloc(maxParticles, sizeof(PartPtr))) == NULL )
    {
        Fire_FuelModelDestroy(catalog, model);
        sprintf(FuelCat_Error(catalog),
            "Fire_FuelModelCreate(): unable to allocate fuel model \"%s\" number %zu for fuel catalog \"%s\".",   /* %d -> %zu (by Undy) */
    ...
    size_t         model;       /* fuel model id number         [0-maxModels] */
{
    ...
    {
        sprintf(FuelCat_Error(catalog),
            "Fire_FuelModelDestroy(): fuel model %zu doesn't exist in fuel catalog \"%s\".",    /* %d -> %zu (by Undy) */
            model, FuelCat_Name(catalog));
        return (FuelCat_Status(catalog) = FIRE_STATUS_ERROR);
    }
...
int
Fire_FuelParticleAdd ( catalog, model, type, load, savr, dens, heat, stot, seff )
    FuelCatalogPtr catalog;     /* FuelCatalogData instance pointer           */
    size_t  model;              /* fuel model id number         [0-maxModels] */
    ...
    double  seff;               /* effective silica content           (lb/lb) */
{
    ...
    {
        sprintf(FuelCat_Error(catalog),
            "Fire_FuelParticleAdd(): fuel model %zu doesn't exist in fuel catalog \"%s\".",    /* %d -> %zu (by Undy) */
            model, FuelCat_Name(catalog));
        return (FuelCat_Status(catalog) = FIRE_STATUS_ERROR);
    }
    ...
    {
        sprintf(FuelCat_Error(catalog),
            "Fire_FuelParticleAdd(): fuel model %zu type value (arg #3) is not FIRE_TYPE_DEAD, FIRE_TYPE_HERB, or FIRE_TYPE_WOOD.",  /* %d -> %zu (by Undy) */
            model);
        return (FuelCat_Status(catalog) = FIRE_STATUS_ERROR);
    }
    ...
    {
        sprintf(FuelCat_Error(catalog),
            "Fire_FuelParticleAdd(): unable to allocate fuel particle to fuel model \"%s\" number %zu in fuel catalog \"%s\".",  /* %d -> %zu (by Undy) */
